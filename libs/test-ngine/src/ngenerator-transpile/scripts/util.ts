...
export function getAngularType(typescript) {
...
export function getEjsTemplate(type) {
  let ejsFile;
  switch (type) {
    case 'Component':
    case 'Directive':
...
export function getImportLib(mports, className) {
  let lib;
...
export function reIndent(str, prefix = "") {
  let toRepl = str.match(/^\n\s+/)[0];
  let regExp = new RegExp(toRepl, 'gm');
  return str.replace(regExp, "\n" + prefix);
}

export function createBackupFile(filePath) {
